...
for i in files:
    if i.endswith(".csv"):
        ...
        #individual graphs
        plt.figure(1)
        #plt.title('HUMATICS vs DW_THALES ' + str(round((UWB_dict[names_array[0]].measurement_array[0])/5)*5) + ' m')
...
if skeleton_key:
    ...
    plt.subplot(2,1,2)
    plt.title('Missed Messages (DECA-Blue vs HUMATICS-Green vs DW_THALES-Red)')
    bar_width = 0.35
    rects1 = plt.bar(index, UWB_dict[names_array[0]].dropped_msg_array, bar_width, color = 'b')
    rects2 = plt.bar(index + bar_width,UWB_dict[names_array[1]].dropped_msg_array, bar_width, color = 'g')
    ...
    print(UWB_dict[names_array[0]].standard_deviation_array)
    #individual standard deviation
    plt.figure(6)
    plt.title('DW API Deviation vs Ground Truth')
    plt.xlabel('Ground Truth (m)')
    plt.ylabel('Distance (m)')
    plt.bar(np.arange(len(ground_truth)), UWB_dict[names_array[0]].standard_deviation_array, align = "center", alpha = 0.5, color = "R")
    ...
    plt.figure(7)
    plt.title("DW API Drop Percentage vs Distance")
    plt.xlabel("Distance (m)")
    plt.ylabel("Drop %")
    plt.bar(np.arange(len(ground_truth)), drop_percent_array, align = "center", alpha = 0.5, color = "R")
    plt.xticks(np.arange(len(ground_truth)), ground_truth)
    plt.legend(["Drop %"])
    plt.grid()
    plt.show()
