...
if skeleton_key:
    ...
    #DW_API vs GT dist
    a, b = lobf(ground_truth, UWB_dict[names_array[0]].avg_dist_array)
    yfit = [a + b * xi for xi in ground_truth]
    plt.figure(5)
    plt.title('DW API Average vs Ground Truth')
    plt.plot(ground_truth, UWB_dict[names_array[0]].avg_dist_array ,'ro')
    plt.plot(ground_truth, yfit, 'r-')
    plt.legend(["DW API Avg Dist", equation(ground_truth, UWB_dict[names_array[0]].avg_dist_array)])
    for x in range (0, len(ground_truth)):
        ground_truth[x] = int((round(ground_truth[x]/5)*5))
    ...
    plt.xlabel('Ground Truth Distance (Truem)')
    plt.ylabel('Distance (m)')
